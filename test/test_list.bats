...
@test "list given an empty bucket succeeds" {
  VCAP_SERVICES="$VCAP_SERVICES" BACKUP_MANAGER_S3_SERVICE_NAME=datastore-backup-test-s3 run list
  assert_success
...
@test "list given a single file succeeds with file listing" {
  ...
  aws_helper s3 cp $BATS_TEST_TMPDIR/test-file s3://$TEST_BACKUP_MANAGER_BUCKET/test-file
  VCAP_SERVICES="$VCAP_SERVICES" BACKUP_MANAGER_S3_SERVICE_NAME=datastore-backup-test-s3 run list
  assert_success
...
@test "list given a path with starting slash succeeds with dir listing" {
  # Create a test file of size 1337
  truncate -s 1337 $BATS_TEST_TMPDIR/test-file
  aws_helper s3 cp $BATS_TEST_TMPDIR/test-file s3://$TEST_BACKUP_MANAGER_BUCKET/test-directory/test-file
  VCAP_SERVICES="$VCAP_SERVICES" BACKUP_MANAGER_S3_SERVICE_NAME=datastore-backup-test-s3 run list /test-directory/
  assert_success
  assert_output --partial "1337 test-file"
}
